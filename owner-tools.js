...
var require_lifecycle = __commonJS({
  "node_modules/firebase-admin/lib/app/lifecycle.js"(exports2) {
    ...
    function deleteApp(app2) {
      return exports2.defaultAppStore.deleteApp(app2);
    }
...
var OWNERS = /* @__PURE__ */ new Set(["ahsanul arifin", "founder", "owner"]);
function app() {
  if (admin.apps.length) return admin.app();
  const serviceAccount = JSON.parse(process.env.FIREBASE_SERVICE_ACCOUNT || "{}");
  return admin.initializeApp({ credential: admin.credential.cert(serviceAccount), databaseURL: process.env.FIREBASE_DATABASE_URL || "https://betachat-v2-default-rtdb.firebaseio.com" });
}
async function ownerDb(idToken) {
  const a = app();
  const decoded = await admin.auth(a).verifyIdToken(idToken);
  const db = admin.database(a);
  const key = (await db.ref(`uids/${decoded.uid}`).once("value")).val();
  if (!OWNERS.has(key)) {
    const e2 = new Error("Owner access required");
    e2.status = 403;
    throw e2;
  }
  return { db, key };
...
exports.handler = async (event) => {
  ...
  try {
    const { idToken, action, text } = JSON.parse(event.body || "{}");
    if (!idToken) throw new Error("Missing sign-in token");
    const { db, key } = await ownerDb(idToken);
    if (action === "metrics") {
      const [usersSnap, presenceSnap, messagesSnap] = await Promise.all([db.ref("usernames").once("value"), db.ref("presence").once("value"), db.ref("messages").once("value")]);
      const today = dayStart(), week = Date.now() - 7 * 864e5;
      let messagesToday = 0, activeWeek = 0;
      const presence = presenceSnap.val() && typeof presenceSnap.val() === "object" ? presenceSnap.val() : {};
      for (const userKey of Object.keys(presence)) {
        const p = presence[userKey];
        if (p && Number(p.last_changed || 0) >= week) activeWeek++;
      }
      const chats = messagesSnap.val() && typeof messagesSnap.val() === "object" ? messagesSnap.val() : {};
      for (const chatCode of Object.keys(chats)) {
        const chat = chats[chatCode];
        if (!chat || typeof chat !== "object") continue;
        for (const messageId of Object.keys(chat)) {
          const m2 = chat[messageId];
          if (m2 && typeof m2 === "object" && Number(m2.ts || 0) >= today && !m2.deleted) messagesToday++;
        }
      }
      return { statusCode: 200, body: JSON.stringify({ users: usersSnap.numChildren(), messagesToday, activeWeek }) };
    }
    if (action === "audit") {
      const item = { actor: key, action: "owner_action", detail: String(text || "").slice(0, 180), ts: Date.now() };
      await db.ref("auditLog").push(item);
      return { statusCode: 200, body: JSON.stringify({ ok: true }) };
    }
    if (action === "broadcast") {
      const message = String(text || "").trim();
      if (!message || message.length > 500) throw new Error("Announcement must be 1\u2013500 characters");
      const users = (await db.ref("usernames").once("value")).val() || {};
      const code = "announcement_" + Date.now();
      const now = Date.now();
      const members = {};
      const updates = {};
      Object.entries(users).forEach(([userKey, name]) => {
        members[userKey] = typeof name === "string" ? name : userKey;
        updates[`usergroups/${userKey}/${code}`] = true;
      });
      updates[`groups/${code}`] = { name: "BetaChat announcement", isBroadcast: true, members, admins: { [key]: true }, createdAt: now, lastActivity: now, lastSender: "BetaChat" };
      updates[`messages/${code}/announcement`] = { sender: "BetaChat", ts: now, text: message, system: true, broadcast: true };
      const auditKey = db.ref("auditLog").push().key;
      updates[`auditLog/${auditKey}`] = { actor: key, action: "broadcast", detail: message.slice(0, 180), ts: now, recipientCount: Object.keys(members).length };
      await db.ref().update(updates);
      return { statusCode: 200, body: JSON.stringify({ ok: true, recipients: Object.keys(members).length }) };
    }
    throw new Error("Unknown action");
  } catch (err) {
    return { statusCode: err.status || 500, body: JSON.stringify({ error: err.message || "Server error" }) };
  }
};
